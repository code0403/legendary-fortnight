...
def round_robin(processes, n, quantum):
    ...
    while True:
        ...
        last_pid = gantt_chart[-1] if gantt_chart else None
        next_pid, current_time = update_queue(processes, n, quantum, ready_queue, current_time)
        
        if next_pid is not None and next_pid != last_pid:
            gantt_chart.append(next_pid)
