...
class AStarSearch:
    def __init__(self, initial_state, goal_state):
        ...
        self.closed_list = set()  # Initialize the closed list as an empty set
    ...
    def a_star_search(self, heuristic):
        ...
        while open_list:
            ...
            self.closed_list.add(current_state)  # Add current state to the closed list
    ...
    def verify_monotonicity(self):
        ...
        for state, successor in zip(list(self.closed_list)[:-1], list(self.closed_list)[1:]):
            if not self.monotone_restriction(state, successor, self.h3):  # Choose a heuristic for verification
                print("Monotone restriction violated.")
                break
        else:
...
for heuristic, heuristic_name in heuristics:
    ...
    search_algorithm.verify_monotonicity()
