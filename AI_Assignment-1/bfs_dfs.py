...
#  Check if the target state is reachable from a given state
def is_reachable(initial_state, target_state, search_algorithm):
    start_time = time.time()  # Record start time
    if search_algorithm == 'BFS':
        result, depth = bfs(initial_state)  # Implementing BFS
    elif search_algorithm == 'DFS':
        result, depth = dfs(initial_state)  # Implementing DFS
    end_time = time.time()  # Record end time
    time_taken = end_time - start_time  # Calculating time taken for each search    
...
#  Find all possible moves from a given state
